...
def evalCompute(dataset_eval, evaldatedapath, fixjson=False):
    ...
    distance_percentage = state_1_rows['eval'].apply(lambda x: x['distance_metricies']).mean()

    # Print the results
    print(f"No- SON-unParsable samples: {state_counts.get(0, 0)}")
    print(f"No- SON-Parsable samples: {state_counts.get(1, 0)}")
    print(f"Average Distance for JSON-Parsable samples: {distance_percentage:.2f}%")
