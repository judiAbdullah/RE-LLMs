...
class javaSequenceGenerator:
    ...
    # recheck for supper like Outer.super() to add Outer ot "To"
    ...
    # def process_IfStmt(self, ifstmt, label=None):
    #     seq = []
    #     ifseq, ifblocks = self.process_IfBlocks(ifstmt)
    #     seq.extend(ifseq)
    #     seq.extend([{
    #                 "type": "blocks",
    ...
    def process_IfStmt(self, block, label=None):
        ...
        # seq = []
        # seq.extend(self.process_expr(expression))
        # ifelseblocks.append({"guard": expression.attrib.get('value'),
        #                     "contents":self.process_stmt(ifblock)})
        
        # if len(if_child) > 2:
        #     if if_child[2].tag == 'IfStmt':
        #         tempseq, temp_blocks = self.process_IfBlocks(if_child[2])
        #         ifelseblocks.extend(temp_blocks)
        #         seq.extend(tempseq)
        #     else:
        #         ifelseblocks.append({"guard": "else",
        #                             "contents":self.process_stmt(if_child[2])})
        # return seq, ifelseblocks
    ...
    def process_expr(self, expr):
        expr_handlers = {
            ...
            # "IntegerLiteralExpr": self.process_IntegerLiteralExpr,
            ...
            # "PatternExpr":self.process_PatternExpr ,
